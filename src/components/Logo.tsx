...
export function Logo({ size = 40 }: { size?: number }) {
  // The gradients are referenced by id, and this mark is rendered several
  // times per page (header, footer, loading skeleton). Shared ids mean the
  // browser resolves every copy against the first one in the document — so
  // when that copy is removed, mid-navigation, the rest lose their fill and
  // flash as an empty square. useId keeps each instance self-contained.
  const uid = useId().replace(/:/g, "");
  const bg = `lws-bg-${uid}`;
  const accent = `lws-accent-${uid}`;
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Learn With Smile">
      ...
      <rect x="3" y="3" width="50" height="50" rx="14" fill={`url(#${bg})`}/>
      ...
      <path d="M16 38 Q28 47 40 38" stroke={`url(#${accent})`} strokeWidth="3" strokeLinecap="round" fill="none"/>
