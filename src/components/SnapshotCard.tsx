      ...
      <div className="relative -rotate-2 rounded-3xl bg-cream shadow-2xl border-4 border-cream/40 p-4 sm:p-5 overflow-hidden">
        ...
        <div className="relative">
          ...
          {(eyebrow || headline || subnote) && (
            <div className="mt-3">
              {eyebrow && (
                <p className="text-ink/85 font-display font-bold text-xs uppercase tracking-wider">
                  {eyebrow}
                </p>
              )}
              {headline && (
                <p className="font-display font-extrabold text-ink leading-none mt-1">
                  ...
                  {headline.suffix && (
                    <span className="text-lg text-ink/85">{headline.suffix}</span>
                  )}
                </p>
              )}
          ...
          <div className="mt-4 space-y-2.5">
            {rows.map((r, i) => (
              <div key={i} className="flex items-center gap-3">
                <span
                  className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${TONE[r.tone]}`}
                >
                  {r.icon}
                </span>
...
export const SnapIcons = {
  ...
  people: ico(
    <>
      ...
      <path d="M3.5 18c.7-2.8 3-4.2 5.5-4.2s4.8 1.4 5.5 4.2" />
      <path d="M14.5 17.5c.4-1.8 2-3 3.5-3s2.5.8 3 2.2" />
    </>,
  ),
  book: ico(
    <>
  ...
  rupee: ico(
    <>
      <path d="M7 6h10M7 10h10M9 6c4 0 4 6 0 6H7l7 7" />
    </>,
  ),
  whatsapp: ico(
    <>
      <path d="M4 20l1.6-4A8 8 0 1 1 8 18.4z" />
      <path d="M9 10.5c.4 1.4 1.6 2.6 3 3l1-1.2 2 1c-.3 1.2-1.5 1.8-2.7 1.5-2.2-.4-4-2.2-4.4-4.4-.3-1.2.3-2.4 1.5-2.7l1 2z" />
    </>,
  ),
};
