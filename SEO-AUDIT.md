...
robots.txt also opens with a comment block pointing AI assistants at the three
files described in §3a before they start crawling.
...
### 3a. The AI-readable layer — llms.txt, llms-full.txt and Markdown mirrors

Prerendering made the text *present*. It did not make it *cheap*: a page is
~60 kB of Tailwind markup around ~4 kB of prose, and answering "how much is
IELTS coaching here" still meant fetching and parsing several of them.

Three files, all generated by `src/lib/llms.ts` from the same `PAGES` /
`COURSES` tables that drive the metadata, so none of them can drift from the
site:

| File | What it is | Size |
|---|---|---|
| `/llms.txt` | The index, in the [llmstxt.org](https://llmstxt.org) format — summary, key facts (fees, batch cap, slots, contact, refund), every course with its price and format, every page with its one-line summary, and an index of all 30 FAQ questions with the page that answers each | ~9 kB |
| `/llms-full.txt` | The readable text of all 14 pages in one file, each section headed by the canonical URL it came from | ~59 kB |
| `/<page>.md` | Per-page Markdown — YAML front matter with the canonical URL, then the page text with nav, footer and markup stripped | 1.2–9.4 kB |

Discovery is wired three ways: the robots.txt comment block, a
`<link rel="alternate" type="text/plain" href="/llms.txt">` site-wide, and a
`<link rel="alternate" type="text/markdown" href="<page>.md">` on each page, so
an assistant that landed on one HTML page can find the text version of exactly
that page.

Notes on the conversion (`htmlToMarkdown` in `src/lib/llms.ts`):

- It is a string pipeline over our own `renderToString` output, not a DOM parse,
  so the prerender step stays dependency-free. The attribute-matching is
  quote-aware because Tailwind arbitrary variants such as `[&>svg]:size-4` put a
  `>` inside a quoted attribute and a naive `[^>]*` would spill markup into the
  prose.
- Elements carrying Tailwind's bare `hidden` class are dropped. They are
  responsive duplicates — the course pages render the fee snapshot twice,
  `hidden lg:block` and `lg:hidden` — and a passage that appears twice in a
  retrieved chunk reads as broken.
- `aria-hidden` icon spans are dropped: they hold Material Symbols ligature
  names, so keeping them would sprinkle `arrow_forward` and `auto_awesome`
  through the text.
- Links are kept and absolutised; WhatsApp links lose their prefilled `?text=`
  message, which is a URL-encoded paragraph that would otherwise dominate.

Every page's mirror must come out over 500 characters or the build fails — the
same guard-rail philosophy as the canonical assertion, aimed at the case where a
markup change silently stops matching.

Deliberately **not** done: linking the `.md` URLs from `llms.txt` instead of the
HTML ones. The rule the file states is "append `.md` to any page URL, but cite
the URL without it", so an assistant fetches the cheap version and still hands
the reader the canonical page.
...
3. `node scripts/prerender.mjs` — renders each route to static HTML, writes
   `sitemap.xml`, `llms.txt`, `llms-full.txt` and a `.md` mirror per page

The prerender step fails the build if any page ends up with zero or multiple
canonical tags or titles, or with a Markdown mirror under 500 characters. The
deploy workflow adds a `Verify crawlability` step that fails if a representative
page ships without a canonical, without structured data, or with an empty
`#root`; if `llms.txt`, `llms-full.txt` or a page mirror ships without fees in
it; or if the number of `.md` mirrors stops matching the number of sitemap URLs
— so a regression back to the blank SPA cannot deploy silently.
